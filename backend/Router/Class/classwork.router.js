...
router.post('/create', jsonParser, (req, res) => {
    ...
    User.findOne({_id: author, token}, (err, user) => {
        ...
        else if (!user) res.status(404).json("Autor não encontrado.")
        else{
            Class.findOne({_id: _class}, (err, __class) => {
                if(err) res.status(500).json("Algo deu errado.")
                else if(!__class) res.status(404).json("Classe não encontrada.")
                else{
                    const newClasswork = new Classwork({title, description, class: _class, types: type, duedate, options, author})
                    newClasswork.save()
                    .then(() => res.json({message: "Trabalho de aula criado.", id: newClasswork._id}))
...
router.get('/class/get/:class', jsonParser, (req, res) => {
    ...
    Class.findOne({_id: classId}, (err, _class) => {
        if(err) res.status(500).json("Algo deu errado.")
            ...
            .catch(() => res.status(500).json("Algo deu errado."))
        }
    })
})
...
router.get('/get/:classwork', jsonParser, (req, res) => {
    ...
    .catch(() => res.status(404).json("Turma não encontrada."))
})
...
router.post('/update/:id', jsonParser, (req, res) => {
    ...
    Classwork.findById(id, (err, classwork) => {
        if(err) res.status(500).json("Algo deu errado.")
        else if(!classwork) res.status(404).json("Turma não encontrada.")
        else{
