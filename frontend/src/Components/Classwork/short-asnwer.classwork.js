...
const ShortAnswer = (params) => {
    ...
    useEffect(() => {
        ...
        .then(res => {
            if(res.data.types === "resposta curta"){
        ...
        <div className="container-fluid">
            ...
            <div className="container margin-top-bottom">
                <div className="col-9">
                    ...
                    {classwork.duedate?<p>Data de Expiração: {moment(classwork.duedate).fromNow()}</p>:null}
                    <p className="box-text material-description">{classwork.description}</p>
                    <p>Postado {moment(classwork.createdAt).fromNow()} 
                    {classwork.createdAt !== classwork.updatedAt? <span>(atualizada {moment(classwork.updatedAt).fromNow()})</span>: null} por {author}</p>
                    {classwork.author === userInfo._id? <div><h3><span className="link" onClick = {openClasswork}>Editar</span></h3>
                    <h3><span className="link text-danger" onClick = {deleteClasswork}>Excluir</span></h3></div>:null}
                </div>
                ...
                <div className="col-3">
                    ...
                    <form className="box box-shadow" onSubmit = {Answer}>
                        <h1 className="box-title">Sua Resposta:</h1>
                ...
                <div className="col-3">
                    <div className="box box-shadow">
                        <h1 className="box-title">Respostas:</h1>
                        <p className="box-text">{classwork.answer?classwork.answer.length:null} respondidas</p>
                        <button className="form-control btn btn-dark" onClick = {showAnswer}>Veja as respostas</button>
                    </div>
                </div>}
            </div>
            ...
            <div className = "classwork-modal" id = "answers">
                <div className="classwork-content container">
                    ...
                    <h1 className="box-title">Respostas de alunos:</h1>
                    {answers.map(answer => {
                        return <p key = {answer._id}>{answer.student.username} respondidas <b>{answer.answer}</b> {moment(answer.answeredOn).fromNow()}
                        {answer.answeredOn > classwork.duedate? <span><b> (Atrasado)</b></span>:null}</p>
                    })}
                </div>
            </div>
            :null}
