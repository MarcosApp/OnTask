                ...
                {classes.map(_class  => {
                    if(!_class.archived && !userInfo.archived_class.includes(_class._id)){
                        return <div className="class box box-shadow" key = {_class._id}>
                            ...
                            <p className="box-option link" onClick = {() => Archive(_class._id, _class.owner)}>Arquivar</p>
                            {_class.students.includes(userInfo._id)?
                                <p className="box-option link" onClick = {() => Unenroll(_class._id)}>Cancelar Inscrição</p>
                            :<p className="box-option link" onClick = {() => window.location = `/class/${_class._id}/setting`}>Configurações</p>}
                        </div>
                    }else return null;
                })}
            </div>
